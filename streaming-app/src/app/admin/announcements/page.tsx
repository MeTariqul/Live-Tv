...
export default function AdminAnnouncementsPage() {
  ...
  const addItem = () => setItems([{ id: generateId(), title: '', content: '', is_pinned: false, published: true, created_at: new Date().toISOString() }, ...items]);
  const removeItem = (id: string) => setItems(items.filter((i) => i.id !== id));
  const updateItem = (id: string, field: string, value: any) => setItems(items.map((i) => i.id === id ? { ...i, [field]: value } : i));
  const save = () => { setAnnouncements(items); toast.success('Announcements saved'); };
          ...
          <motion.div key={item.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}>
            <Card><CardContent className="p-4 space-y-3">
              <div className="flex items-center gap-2">
                ...
                <Button variant="ghost" size="icon" onClick={() => updateItem(item.id, 'is_pinned', !item.is_pinned)}><Pin className={`h-4 w-4 ${item.is_pinned ? 'text-yellow-500' : ''}`} /></Button>
