...
export default function AdminSchedulePage() {
  ...
  const [entries, setEntries] = useState<any[]>(schedule || []);

  const addEntry = () => {
    setEntries([...entries, { id: Date.now().toString(), day: 'Monday', start_time: '18:00', end_time: '22:00', title: 'Gaming Session', game: '', is_active: true }]);
  };
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex gap-2">
          ...
          <Button size="sm" onClick={() => toast.success('Schedule saved')}><Save className="mr-2 h-4 w-4" />Save</Button>
        </div>
      </div>
          ...
          <motion.div key={entry.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}>
            <Card>
              <CardContent className="p-4 flex flex-wrap items-center gap-3">
                <select value={entry.day} onChange={(e) => updateEntry(entry.id, 'day', e.target.value)} className="rounded-lg border bg-background px-3 py-2 text-sm">
