...
import { useStream, useSchedule, useRecordings } from '@/hooks/use-data';
import { SITE_NAME } from '@/lib/constants';
import { formatDate, getYouTubeThumbnail } from '@/lib/utils';

export default function HomePage() {
  const { data: stream, isLoading: streamLoading } = useStream();
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6">
      ...
      <motion.section
        ...
        className="relative mb-12 overflow-hidden rounded-2xl bg-gradient-to-br from-red-500/10 via-background to-purple-500/10 p-8 sm:p-12"
      >
        <div className="relative z-10">
          <div className="flex items-center gap-3 mb-4">
            {isLive ? (
              ...
              <Badge variant="secondary">
                <Clock className="mr-1 h-3 w-3" /> Offline
              </Badge>
            )}
          </div>
          ...
          <p className="text-lg text-muted-foreground mb-6 max-w-2xl">
            {stream?.description || 'Watch live streams, past broadcasts, and stay updated with the latest schedule.'}
          </p>
          <div className="flex flex-wrap gap-3">
            {isLive ? (
              <Link href="/live">
                <Button size="lg" className="bg-red-500 hover:bg-red-600 text-white">
                  <Radio className="mr-2 h-4 w-4" /> Watch Live
                </Button>
              </Link>
            ) : nextStream ? (
              <Link href="/schedule">
            ...
            <Link href="/streams">
              <Button size="lg" variant="outline">
                <Play className="mr-2 h-4 w-4" /> Past Streams
              </Button>
            </Link>
          </div>
        </div>
                    ...
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      {stream?.viewer_count != null && stream.viewer_count > 0 && (
                        <span className="flex items-center gap-1 text-xs text-white/80">
                          <Users className="h-3 w-3" /> {stream.viewer_count.toLocaleString()}
                        </span>
                      )}
                    </div>
                    <h2 className="text-xl font-bold text-white">{stream?.title || 'Live Stream'}</h2>
                    {stream?.game && <p className="text-sm text-white/70">{stream.game}</p>}
                  </div>
                  <div className="absolute top-4 right-4">
                    <div className="flex items-center gap-1 rounded-full bg-black/50 px-3 py-1 text-xs text-white backdrop-blur">
      ...
      <section className="mb-12 grid grid-cols-2 gap-4 sm:grid-cols-4">
        {[
          { icon: Radio, label: 'Streams', value: recordings?.length ?? 0 },
          { icon: Users, label: 'Total Views', value: recordings?.reduce((a, r) => a + (r.view_count || 0), 0).toLocaleString() ?? '0' },
          ...
          >
            <Card>
              <CardContent className="p-4 text-center">
                <stat.icon className="mx-auto h-5 w-5 text-muted-foreground mb-2" />
      ...
      {recordings && recordings.length > 0 && (
        <section className="mb-12">
          <div className="flex items-center justify-between mb-6">
            ...
            <Link href="/streams" className="text-sm text-muted-foreground hover:text-foreground flex items-center gap-1">
              View all <ArrowRight className="h-3 w-3" />
            </Link>
          </div>
              ...
              <Link key={rec.id} href={`/streams#${rec.id}`}>
                <Card className="overflow-hidden hover:border-primary/40 transition-colors group">
                  ...
                  <CardContent className="p-4">
                    ...
                    <p className="text-xs text-muted-foreground mt-1">{formatDate(rec.created_at)}</p>
                  </CardContent>
                </Card>
              </Link>
            ))}
